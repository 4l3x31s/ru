...
angular.module('starter.services', [])
  .factory("ServiciosRuat",function($http, $q, ApiEndpoint,$rootScope, $ionicPopup){
    return {
      getPostGlobal : function(objeto, urlModulo:string){
        var defered = $q.defer();
        ...
        //$http.post( ApiEndpoint.url + urlModulo, objeto, config)
        ...
        return promise;      },
      getPostPdf : function(objeto, urlModulo:string){
        var defered = $q.defer();
        var promise = defered.promise;
        var config = {
          headers : {
      ...
      getGlobal: function(urlModulo:string){
        var defered = $q.defer();
        var promise = defered.promise;
        var url = $rootScope.urlServicio;
        console.log("URL: Final: "+ url + urlModulo);
        $http.get(url + urlModulo)
        //$http.get(ApiEndpoint.url + urlModulo)
          .success(function(data,status, headers, config) {
            var respuestaServicio = {
              data   : data,
              status : status
      ...
      getStatus: function (status:number) {
        var mensaje = "";
        switch (status){
          case 400://BadRequest
